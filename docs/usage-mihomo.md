...
- 本地私有 Mihomo 配置里，所有基于 provider 的代理组默认共用同一套排除条件：过滤 `剩余流量`、`直接连接`、`FlyintPro` 独立占位项、`套餐到期`、`距离下次重置`、`联系我们`、`过滤掉`、`Expire Data`、`Traffic Reset`，以及 `1.2 GB | 50 GB` 这类流量统计提示，让手动切换、自动组和地区组尽量只展示真实节点。
- 如果还需要过滤某个供应商名或 Provider 占位项，只对独立项全名做精确匹配，不要直接拿供应商名做宽泛子串匹配；否则一旦节点侧额外注入统一前缀，真实节点也会被一起误杀。
