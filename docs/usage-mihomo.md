...
- 本地私有 Mihomo 配置里，所有基于 provider 的代理组默认共用同一套排除条件：`剩余流量`、`直接连接`、`FlyintPro`、`套餐到期`、`距离下次重置`、`联系我们`、`过滤掉`、`Expire Date`、`Traffic Reset` 与 `1.2 GB | 50 GB` 这类流量提示统一按任意位置命中即过滤，让手动切换、自动组和地区组尽量只展示真实节点。
- 这套过滤条件需要在所有相关代理组里保持完全一致；Mihomo 侧所有相关 `exclude-filter` 都必须与 Surge 语义对齐。如果未来需要恢复保留带统一前缀的真实节点，必须与 Surge 一起同步收窄两侧表达式。
