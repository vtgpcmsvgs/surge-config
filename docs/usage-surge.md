...
- 在 Surge 中接入 `reject/adspower_reject.list`、`direct/adspower_direct.list` 与 `proxy/adspower_proxy.list` 时，三条 `RULE-SET` 都应追加 `extended-matching`，以匹配目标为 IP、但 TLS SNI / HTTP Host 仍为 AdsPower 域名的连接。
