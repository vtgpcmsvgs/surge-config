...
- 本地私有 Surge 配置里，所有基于 `policy-path` 的代理组默认共用同一套排除条件：过滤 `剩余流量`、`直接连接`、`FlyintPro` 独立占位项、`套餐到期`、`距离下次重置`、`联系我们`、`Expire Data`、`Traffic Reset`，以及 `1.2 GB | 50 GB` 这类流量统计提示，让手动切换、自动组和地区组尽量只展示真实节点。
